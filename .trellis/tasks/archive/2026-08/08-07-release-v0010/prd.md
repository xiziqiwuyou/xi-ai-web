...
- [x] Package, lockfile, Compose templates, README, and release notes identify
      `v0.0.10` consistently.
- [x] Release notes do not claim real-provider, Docker, deployed-online, or
      physical-device verification without evidence.
- [x] Type-check, provider contracts, feature audit, privacy, server tests,
      build, release-check, and focused Admin E2E pass.
- [x] No API Key, proxy credential, image output, or deployment secret enters
      the commit or tag.
- [x] Remote `master` contains release bookkeeping and remote `v0.0.10`
