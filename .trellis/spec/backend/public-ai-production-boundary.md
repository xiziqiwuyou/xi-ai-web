...
- The root Compose deployment is an image consumer, never a source builder. It pulls `XI_AI_WEB_IMAGE` (default `ghcr.io/xiziqiwuyou/xi-ai-web:latest`) for the Web process, knowledge migration, and knowledge worker so all roles run the same release. GitHub Actions publishes reviewed `linux/amd64` and `linux/arm64` images with `latest`, commit-SHA, and `v*` tags; production may pin an immutable tag for rollback.
