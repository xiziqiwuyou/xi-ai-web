...
- `SearchServiceConfig` is shared request/session data, never a workspace record. `ChatStreamPayload.searchService` and `AgentRunPayload.searchService` are optional and may be projected only when `allowedTools` contains `web_search`.
...
- Admin labels the fields `前台显示名称` and `实际请求模型名`, validates them independently, and shows a `label -> vendor / model` preview.
...
- Admin E2E: assert both fields, mapping preview, independent empty-field errors, and no save request for invalid data.
