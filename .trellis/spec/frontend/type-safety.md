...
- `sanitizeArtifact()` accepts only the four artifact kinds, bounded title/language/content/provenance, positive safe version numbers, and non-empty records; it removes duplicate version numbers and duplicate version IDs before a record reaches React, IndexedDB, workspace archives, or export.
