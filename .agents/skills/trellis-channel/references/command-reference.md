...
> Multi-agent collaboration runtime 鈥?spawn / coordinate / interrupt worker
...
Behavior:
- Appends a `create` event; immutable `type` (cannot mutate forum鈫攃hat after).
...
- `--kind` is validated against `CHANNEL_EVENT_KINDS` (single value, not
  CSV 鈥?that's the `wait` side).

### `wait <name>`

```bash
trellis channel wait <name>
  --as <agent>                            # REQUIRED 鈥?self for filter ctx
  [--scope project|global]
...
- Default `--to` filter is the caller's own agent (broadcast events still
  match 鈥?broadcast + explicit-to-me).
...
## tag-vs-kind 鈥?how event shape is actually controlled
...
- `--kind` is the only event-type filter, and it is constrained to the
  ...
  - Passing anything else throws
    `Invalid --kind '<x>'. Must be one of: 鈥.
- `--kind` lives on `wait` (CSV, OR semantics) and `messages` (single
  value). `send` and `run` cannot emit a custom kind 鈥?every `send` writes
  a `message` event.
...
- Use `--kind done,turn_finished` for "worker finished a turn" 鈥?these are
...
trellis channel interrupt <name> [text]
  --as <agent>                            # REQUIRED 鈥?caller
  --to <agent>                            # REQUIRED 鈥?target worker
  [--scope project|global]
...
- Records a `spawned` event with `pid`, `provider`, `agent`, `files`,
  `manifests`.
- OOM-guard precedence: CLI flag 鈫?env var
  (`TRELLIS_CHANNEL_WORKER_IDLE_TIMEOUT`,
  `TRELLIS_CHANNEL_MAX_LIVE_WORKERS`) 鈫?  `.trellis/config.yaml#channel.worker_guard` 鈫?built-in defaults.
...
trellis channel kill <name>
  --as <agent>                            # REQUIRED 鈥?worker agent name
  [--scope project|global]
  [--force]                               # SIGKILL immediately
```

Behavior:
- Default path: SIGTERM 鈫?8 s grace 鈫?SIGKILL escalation; the CLI writes a
...
Behavior:
- Filter flags are mutually exclusive 鈥?error otherwise.
...
- `<action>` is free-form on the CLI surface; conventional values include
  ...
  `processed`.
- `action=rename` is rejected 鈥?use `thread rename` instead.
...
| `channel __parse-trace <adapter> <file>` | Dev helper 鈥?replays a recorded stream-json / wire trace through the matching adapter and prints the resulting channel events. Adapter is validated against the provider registry. |
